...
- `prompts/analyst_prompt.py` — `ANALYST_SYSTEM_PROMPT` (the guardrails) and
  the shared `NOT_FOUND = "Data not found"` constant. The Discovery prompt
  still lives in `graph/nodes.py`.
- `tests/test_graph_smoke.py` — offline pytest smoke test: fakes the search
  tools and `_get_llm`, runs the compiled graph, and asserts 3 valid
